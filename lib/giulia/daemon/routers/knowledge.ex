defmodule Giulia.Daemon.Routers.Knowledge do
  ...
  post "/pre_impact_check" do
    ...
    if path && module && action do
      ...
      # Facade forwards the raw map to Store and stamps :schema_version on the
      # way out — single source, shared with MCP. Input path unchanged.
      case Giulia.Knowledge.Facade.pre_impact_check(resolved_path, conn.body_params) do
        {:ok, result} ->
          send_json(conn, 200, result)
